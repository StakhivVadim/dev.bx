	...
	<div class="sidebar">
		<ul class="menu">
			...
			foreach ($config['menu'] as $code => $name): ?>
				<div class="menu-item">
					<a class="menu-item-link <?= (isset($_GET['genre']) === false and $currentPage === $code) ? "active"
						: "" ?>" href="<?= $code . ".php" ?>"><?= $name ?></a>
				</div>
			<?php
			endforeach; ?>
			<?php
			foreach ($config['genres'] as $genre => $nameg): ?>
				<div class="menu-item">
					<a class="menu-item-link <?= ($_GET['genre']) === $genre ? "active"
						: "" ?>" href="<?= "index.php?genre=" . $genre ?>"><?= $nameg ?></a>
				</div>
			<?php
			endforeach; ?>
	</div>
